...
The core research question is: *how well does a zero-shot NLI model trained on general English text generalise to the domain of tech news classification, with no fine-tuning?* We answer this using the India Headlines News Dataset as an offline evaluation benchmark.
...
RSS feeds → feedparser → 30 articles
    ↓
bart-large-mnli (zero-shot NLI) → subcategory label + confidence score
    ↓
Llama 3.3 70B via Groq API → 3-bullet summary (on demand, cached 1 hr)
    ↓
...
The confidence distribution plot shows that correct predictions cluster near high confidence (>0.85), while incorrect predictions are spread more uniformly. This indicates the model's confidence score is a reliable signal for downstream uncertainty flagging — the UI uses it to colour-code each article card (green > 80%, amber > 60%, grey otherwise).
...
2. **Summarisation hallucination:** Llama 3.3 70B may generate plausible-sounding but inaccurate bullet points when the article body is short or behind a paywall.
...
4. **Rate limits:** The Groq free tier enforces per-minute request limits. Summaries are generated lazily (on click) and cached for one hour to mitigate this.
...
6. **Keyword pseudo-labels:** The subcategory distribution uses heuristic keyword matching as approximate reference — it is a diagnostic tool, not a strict benchmark.
...
Code scaffolding assisted by **Claude (Anthropic)**. All evaluation, result analysis, and written interpretation are our own. The Groq API (Llama 3.3 70B) is used at runtime for article summarisation.
